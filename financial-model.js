/* Board financial dashboard model: YTD April P&L, reallocations and forecast logic. */
const COLORS = {
  general: '#1E3A8A', life: '#059669', outsourcing: '#7C3AED', accent: '#2563EB',
  costs: '#DC2626', shared: '#9CA3AF', nav: '#1F2937'
};
...
function money(value, compact = false) {
  return new Intl.NumberFormat('en-AU', { style: 'currency', currency: 'AUD', maximumFractionDigits: compact ? 0 : 2, notation: compact ? 'compact' : 'standard' }).format(value || 0);
}
...
function buildBusinessModel() {
  ...
  const beforeProfit = (directRevenue.general + directRevenue.life + sharedRevenue) - (directCosts.general + directCosts.life + sharedCostsBeforeCarveOut);
  ...
  return { data, businesses, sharedRevenue, sharedCostsBeforeCarveOut, sharedCosts, outsourcingCost, beforeProfit, afterProfit, continuingProfit, validationDelta: afterProfit - beforeProfit };
}
...
function linesFor(items, column) { return items.map((item) => ({ name: item.name, group: item.group, category: item.category, value: item[column] || 0 })).filter((item) => Math.abs(item.value) > 0.001); }

Chart.defaults.font.family = 'Inter, ui-sans-serif, system-ui, sans-serif';
Chart.defaults.color = '#4B5563';
Chart.defaults.plugins.tooltip.backgroundColor = '#111827';
if (window.ChartDataLabels) Chart.register(ChartDataLabels);
const labelPluginConfig = { color: '#111827', anchor: 'end', align: 'top', clamp: true, clip: false, font: { size: 10, weight: '700' }, formatter: (v) => Math.abs(v) < 1 ? '' : money(v, true) };
function chartOptions(stacked = false) {
  return { responsive: true, maintainAspectRatio: false, animation: { duration: 550 }, plugins: { legend: { position: 'bottom', labels: { usePointStyle: true, boxWidth: 8 } }, datalabels: labelPluginConfig, tooltip: { callbacks: { label: (ctx) => `${ctx.dataset.label}: ${money(ctx.parsed.y ?? ctx.parsed)}` } } }, scales: { x: { stacked, grid: { display: false } }, y: { stacked, ticks: { callback: (v) => money(v, true) }, grid: { color: '#EEF2F7' } } } };
}
